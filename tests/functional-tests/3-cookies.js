...
describe('Cookies', function() {
	...
	it('cookie object should not exist on not-logged in clients', async () => {
		const hereCookies = await pageC.evaluate(() => webstrate.user.cookies);
		assert.isUndefined(hereCookies);
	});

	it('should be no here cookies on logged in clients', async () => {
		const hereCookiesA = await pageA.evaluate(() => webstrate.user.cookies.here.get());
		assert.isEmpty(hereCookiesA);

		const hereCookiesB = await pageB.evaluate(() => webstrate.user.cookies.here.get());
	...
	it('should be able to set and read here cookie from client setting cookie', async () => {
		await pageA.evaluate((cookieValue1) =>
			webstrate.user.cookies.here.set('__test_1', cookieValue1), cookieValue1);

		await util.waitForFunction(pageA, () => webstrate.user.cookies.here.get('__test_1'));

		const hereCookies = await pageA.evaluate(() => webstrate.user.cookies.here.get());
		assert.deepEqual(hereCookies, { __test_1: cookieValue1 });
	});

	it('should be able to read here cookie from other tab/page', async () => {
		await util.waitForFunction(pageB, () => webstrate.user.cookies.here.get('__test_1'));

		const hereCookies = await pageB.evaluate(() => webstrate.user.cookies.here.get());
		assert.deepEqual(hereCookies, { __test_1: cookieValue1 });
	});
	...
	it('should be able to set cookieUpdateHere event listener on all clients', async () => {
		await pageA.evaluate(() => {
			...
			webstrate.on('cookieUpdateHere', (key, value) =>  window.__test_cookie = [key, value]);
		});
	...
	it('setting cookie should trigger cookieUpdateHere event on client setting cookie', async () => {
		await pageA.evaluate((cookieValue2) =>
			webstrate.user.cookies.here.set('__test_2', cookieValue2), cookieValue2);
	...
	it('here cookies should be accessible on other client after login', async () => {
		...
		const hereCookies = await pageC.evaluate(() => webstrate.user.cookies.here.get());
		assert.deepEqual(hereCookies, {
			__test_1: cookieValue1,
			__test_2: cookieValue2
		});
	...
	// pageA: now on another webstrate.
	// pageB, pageC: still on initial webstrate.
	//
	// We can't test for 'no anywhere cookies', as the user account we're using might be using
	// anywhere cookies.
	it('should be no here cookies on other webstrate', async () => {
	...
	it('should be able to set cookieUpdateAnywhere event listener on all clients', async () => {
		await Promise.all([
			pageA.evaluate(() => {
				window.__test_cookie = false;
				webstrate.on('cookieUpdateAnywhere', (key, value) =>  window.__test_cookie = [key, value])
			}),
			pageB.evaluate(() => {
				window.__test_cookie = false;
				webstrate.on('cookieUpdateAnywhere', (key, value) =>  window.__test_cookie = [key, value])
	...
	it('should be able to set and read anywhere cookie from client setting cookie', async () => {
		await pageA.evaluate((cookieValue3) =>
			webstrate.user.cookies.anywhere.set('__test_3', cookieValue3), cookieValue3);

		const cookieSet = await util.waitForFunction(pageA, () =>
			webstrate.user.cookies.anywhere.get('__test_3'));
		assert.isTrue(cookieSet);
	});
	...
	it('setting cookie should trigger cookieUpdateAnywhere event on client setting cookie', async () => {
		await pageA.evaluate((cookieValue3) =>
			webstrate.user.cookies.anywhere.set('__test_3', cookieValue3), cookieValue3);
	...
	it('cookieUpdateAnywhere should trigger with correct values on client setting cookie', async () => {
		const [cookieKey, cookieValue] = await pageA.evaluate(() => window.__test_cookie);
