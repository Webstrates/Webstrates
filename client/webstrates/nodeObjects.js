...
coreEvents.addEventListener('populated', targetElement => {
	coreUtils.recursiveForEach(targetElement, childNode => {
		// If we run a static version we do not want to set wids on elements as the
		// coreDatabase does not have a doc variable to look them up.
		if (!window.webstrate.isStatic) {
